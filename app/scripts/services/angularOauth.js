/** FORK FROM https://github.com/enginous/angular-oauth **/
  ...
  .provider('OAuth', function() {
    ...
    this.$get = function($q, $window, $rootScope) {
      ...
      if (requiredAndMissing.length) {
        throw new Error('oauth is insufficiently configured.  Please ' +
          'configure the following options using ' +
          'oauth.extendConfig: ' + requiredAndMissing.join(', '));
      }

      if (!config.clientId) {
        throw new Error('clientId needs to be configured using oauth.');
      }
      ...
      return {
        ...
        getToken: function() {
          ...
          else if ( !!(localToken = localStorage[config.localStorageName])) {
            return localToken;
          }
          else {
            return false;
         ...
         * @param accessToken
         */
        setToken: function(accessTokenSet) {
          accessToken = accessTokenSet;
          localStorage[config.localStorageName] = accessTokenSet;
        },
         ...
         */
        verifyTokenAsync: function(accessToken) {
        ...
        getTokenByPopup: function(extraParams, popupOptions) {
          ...
          angular.element($window).on('message.oAuth', function(event) {
            ...
            angular.element($window).off('message.oAuth');
          });
